...
public class KThread {
	...
	public KThread() {
		...
		waitThreads = ThreadedKernel.scheduler.newThreadQueue(true);
		waitThreads.acquire(this);
	...
	public void join() {
		...
		// // Make dummy variable to hold KThread location
		// KThread waitThread = null;

		// Check if the joinee is not finished
		if (status != statusFinished) {
			
			// If not, sleep
			waitThreads.waitForAccess(this.currentThread());
			KThread.sleep();
	...
	/**
	 *	Helper method used to wake all the threads that were 
	 *	joined to the thread.
	 */
	private void unjoin() {
		...
		// Continue till the thread is not null
		do {
			thread = waitThreads.nextThread();

			// If not null, ready it
			if (thread != null)
				thread.ready();
	...
	private ThreadQueue waitThreads = null;
}
